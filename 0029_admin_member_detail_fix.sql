...
-- Supersedes 0021: that version ordered subscriptions by created_at, a column
-- that table does not have, so every call failed with a 400.
--
...
begin
  ...
  select jsonb_build_object(
    ...
    'products', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'document_path', (select m.storage_path from media m
          where m.product_id = p.id and m.kind = 'document' order by m.created_at desc limit 1))
    ...
    -- subscriptions has no created_at; the newest is the one ending last
    'subscription', (select to_jsonb(s) - 'account_id' from subscriptions s
      where s.account_id = a.id order by s.ends_on desc nulls last limit 1),
