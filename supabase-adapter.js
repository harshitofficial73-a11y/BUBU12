...
(function () {
  ...
  // ── Phone and email normalising, shared by OTP and registration ───────────
  //
  // A Ugandan number arrives half a dozen ways: 0776007755, 776007755,
  // +256776007755, 256776007755, with spaces or dashes. Supabase wants E.164
  // and rejects anything else with a message a trader cannot act on, so it is
  // normalised here rather than at each call site.
  //
  // India is included because you are testing from there.
  const normalisePhone = (raw) => {
    let d = String(raw || '').replace(/[^\d+]/g, '');
    if (d.startsWith('+')) d = d.slice(1);
    d = d.replace(/\D/g, '');
    if (!d) return { ok: false, why: 'Enter your mobile number.' };
    // already a country code we know
    if (d.startsWith('256') && d.length === 12) return { ok: true, e164: '+' + d };
    if (d.startsWith('91') && d.length === 12) return { ok: true, e164: '+' + d };
    // local Ugandan: 0776007755 or 776007755
    if (d.startsWith('0')) d = d.slice(1);
    if (d.length === 9 && /^[347]/.test(d)) return { ok: true, e164: '+256' + d };
    if (d.length === 10 && /^[6-9]/.test(d)) return { ok: true, e164: '+91' + d };
    if (d.length < 9) return { ok: false, why: 'That number is too short. A Ugandan mobile has 9 digits after the 0.' };
    if (d.length > 12) return { ok: false, why: 'That number has too many digits. Check for a repeated country code.' };
    return { ok: false, why: 'That does not look like a mobile number. Try 0776 007755 or +256 776 007755.' };
  };
  const looksLikeEmail = (raw) => /^[^\s@]+@[^\s@]+\.[a-z]{2,}$/i.test(String(raw || '').trim());

  // Is this failure the PLATFORM's fault rather than the trader's? A missing
  // SMS provider or unconfigured SMTP must not stop somebody registering — they
  // did nothing wrong and can do nothing about it. A wrong code or a taken
  // email is theirs to fix, and must not be waved through.
  const isDeliveryFault = (err) => {
    const m = String((err && (err.message || err.msg)) || err || '').toLowerCase();
    return m.includes('unsupported phone provider')
      || m.includes('phone provider')
      || (m.includes('sms') && (m.includes('provider') || m.includes('not enabled')))
  ...
  // Supabase returns terse, developer-facing messages. A trader needs to know
  // what to do next, so the ones they will actually hit are translated. Anything
  // unrecognised is passed through rather than replaced with a vague apology —
  // an odd message is more use than no message.
  const humanAuthError = (err) => {
    const raw = String((err && (err.message || err.msg)) || err || '').trim();
    ...
    if (m.includes('user already exists'))
      return 'That number or email already has an account. Sign in instead.';
    // Raw Postgres constraint names reach the trader otherwise — this is what
    // they see when their number is already on another account, which happens
    // whenever a business registers twice.
    if (m.includes('accounts_phone_key') || (m.includes('duplicate key') && m.includes('phone')))
      return 'That mobile number is already on a BUBU account. Sign in instead, or use another number.';
    if (m.includes('accounts_email_key') || (m.includes('duplicate key') && m.includes('email')))
      return 'That email is already on a BUBU account. Sign in instead.';
    if (m.includes('duplicate key') || m.includes('unique constraint'))
      return 'Some of those details are already on another BUBU account. Sign in instead.';
    if (m.includes('invalid login credentials'))
      return 'That email and password do not match an account.';
    if (m.includes('token has expired') || m.includes('otp_expired'))
      return 'That code has expired. Ask for a new one.';
    if (m.includes('invalid otp') || m.includes('token is invalid') || m.includes('invalid token'))
      return 'That code is not right. Check the digits and try again.';
    if (m.includes('sms') && (m.includes('provider') || m.includes('not enabled')))
      return 'Codes by phone are not switched on yet. Use the email code instead.';
    // Supabase's wording for a missing SMTP sender, and for OTP signups being
    // disabled. A trader has no idea what a magic link is, and neither is
    // their fault — both mean the platform cannot deliver a code right now.
    if (m.includes('error sending magic link') || m.includes('error sending confirmation')
        || m.includes('error sending recovery'))
      return 'We could not send that email. Registration will carry on without a code.';
    if (m.includes('signups not allowed for otp'))
      return 'Codes are not switched on for new accounts yet. Registration will carry on without one.';
    if (m.includes('smtp') || m.includes('email provider'))
      return 'Email sending is not configured yet. Registration will carry on without a code.';
    ...
    if (m.includes('invalid phone') || m.includes('phone number format'))
      return 'That number was rejected. Use the full form, e.g. +256 776 007755.';
    if (m.includes('email rate limit') || m.includes('over_email_send_rate_limit'))
      return 'Too many codes sent to that address. Wait a few minutes and try again.';
    ...
    if (m.includes('signups not allowed') || m.includes('signup is disabled'))
      return 'New accounts are closed at the moment. Contact BUBU support.';
    if (m.includes('email not confirmed'))
      return 'Confirm your email first — check your inbox for the code.';
    if (m.includes('failed to fetch') || m.includes('network'))
      return 'No connection to BUBU. Check your data and try again.';
    if (m.includes('captcha')) return 'The security check failed. Reload the page and try again.';
  ...
  const auth = {
    ...
    humanAuthError,
    isDeliveryFault,
    // WhatsApp delivery is a Supabase setting, not a different call: with Twilio
    // Verify configured for WhatsApp, passing channel 'whatsapp' sends the code
    // there. It falls back to SMS when the provider does not support it, so the
    // caller does not have to know which is configured.
    requestOtp(phone, opts) {
      const o = opts || {};
      const p = normalisePhone(phone);
      if (!p.ok) return Promise.resolve({ error: { message: p.why } });
      const options = { shouldCreateUser: o.create !== false };
      if (o.channel === 'whatsapp') options.channel = 'whatsapp';
      return client().auth.signInWithOtp({ phone: p.e164, options })
        .then(res => {
          // Not every provider carries WhatsApp. Retrying as SMS is better than
          // telling a trader their number is wrong when it is not.
          // Retry as SMS only when WHATSAPP specifically was refused. Matching
          // 'unsupported' caught "Unsupported phone provider" too, so a project
          // with no provider at all made two failing calls before falling
          // through — a real wait on a slow connection.
          const why = String(res && res.error && res.error.message || '');
          if (res && res.error && o.channel === 'whatsapp'
              && /whatsapp/i.test(why)
              && !/unsupported phone provider|phone provider/i.test(why)) {
            return client().auth.signInWithOtp({ phone: p.e164,
              options: { shouldCreateUser: o.create !== false } })
              .then(r2 => ({ ...r2, fellBackToSms: !r2.error }));
          }
          return res;
        });
    },
    requestEmailOtp(email, create) {
      if (!looksLikeEmail(email)) {
        return Promise.resolve({ error: { message: 'Enter a full email address, e.g. name@company.co.ug' } });
      }
      return client().auth.signInWithOtp({ email: String(email).trim(),
        options: { shouldCreateUser: create !== false } });
    },
    async verifyOtp(target, token, channel) {
      const code = String(token || '').replace(/\D/g, '');
      if (code.length < 4) throw new Error('Enter the code you were sent.');
      if (channel === 'email') {
        return dataOf(await client().auth.verifyOtp({ email: String(target).trim(), token: code, type: 'email' }));
      }
      const p = normalisePhone(target);
      if (!p.ok) throw new Error(p.why);
      return dataOf(await client().auth.verifyOtp({ phone: p.e164, token: code, type: 'sms' }));
    },
    // After an OTP has proved who they are, this sets the password so they can
    // sign in normally next time rather than needing a code every visit.
  ...
  async function offersFor(productId) {
    // Without this, a missing id reached PostgREST as the literal string
    // "undefined" and came back 400 — ten products on a page meant ten failed
    // requests and a console full of red. There is nothing to look up without
    // an id, so answer that here rather than asking the server.
    if (!productId) return [];
    return (dataOf(await sb.from('product_offers').select('*').eq('product_id', productId)) || []).map(o => ([
      o.supplier, o.district_id, Number(o.price),
      // MOQ and unit kept apart, so the caller can plural it properly rather
      // than printing "1 units".
      Number(o.moq || 1), null, !!o.verified,
      Number(o.years_on_platform || 0), o.supplier_id,
      // Real figures, replacing the ones the card used to invent from price
      // and list position.
      Number(o.supplier_rating != null ? o.supplier_rating : 5),
      Number(o.supplier_rating_count || 0),
      Number(o.response_rate != null ? o.response_rate : 100),
      o.unit || 'unit'
    ]));
  }
  ...
  // A phone field in this database holds all sorts: several numbers run
  // together with no separator ("+256717290587+256700290587+256709015041"),
  // a digit too many, a national 0-prefix. wa.me and tel: both need ONE
  // number in international form, so every link goes through this.
  //
  // Returns '' when nothing usable is there, which is the signal to hide the
  // button rather than render one that cannot work.
  function ugPhone(raw) {
    let s = String(raw == null ? '' : raw).trim();
    if (!s) return '';
    // Ugandan numbers are 256 then 9 digits beginning 7 (mobile) or 3/4
    // (fixed). Checking the prefix matters: without it, a mistyped
    // "+256291397131112" truncates to 256291397131 — a plausible-looking
    // number that belongs to nobody. Dialling a stranger is worse than
    // showing no button.
    const ok = (d) => /^256[734]\d{8}$/.test(d);
    const parts = s.split('+').filter(Boolean);
    for (const part of (parts.length ? parts : [s])) {
      let d = part.replace(/[^0-9]/g, '');
      if (!d) continue;
      if (d.startsWith('256') && ok(d.slice(0, 12))) return d.slice(0, 12);
      if (d.startsWith('0') && ok('256' + d.slice(1, 10))) return '256' + d.slice(1, 10);
      if (d.length === 9 && ok('256' + d)) return '256' + d;
    }
    return '';
  }
  // The ring list. A purpose-built table with owner_label and per-handset
  // hours, so none of it has to be squeezed into accounts.alt_phone.
  async function addHandset(phone, ownerLabel) {
    const a = await accountRow();
    if (!a) throw new Error('Sign in first');
    const rows = dataOf(await sb.from('handsets').select('id').eq('account_id', a.id)) || [];
    if (rows.length >= 5) throw new Error('Five numbers is the limit. Remove one first.');
    return dataOf(await sb.from('handsets')
      .insert({ account_id: a.id, phone, owner_label: ownerLabel || null })
      .select().single());
  }
  const removeHandset = (id) => sb.from('handsets').delete().eq('id', id);
  const setHandsetHours = (id, patch) => sb.from('handsets').update(patch).eq('id', id);
  ...
  // The next questions to put to a buyer, shaped to the product they named.
  // Answers so far go with the request, so an answer can change what follows.
  async function askRequirement(product, answers) {
    try {
      const r = await callAi('ask', { product: product || '', answers: answers || {} });
      if (!r || r.error) return { questions: [], done: true, offline: true };
      return { questions: r.questions || [], done: r.done !== false };
    } catch (_) {
      // A buyer must never be stuck behind a question we could not fetch.
      return { questions: [], done: true, offline: true };
    }
  }
  ...
  window.BUBU_API = { client: sb, auth, bootstrap, loadAccount, loadCategories, searchProducts, productsInCategory, productById, saveProduct,
    ...
    loadDistricts, suggestProducts, askRequirement, submitFinanceEnquiry, supplierProfile, listingsForProduct,
    myRequirements, allotSuppliers, rateMatch, closeRequirement, submitLogisticsRequest,
    myCapabilities, applyToSell, decideSeller, myLeadBoard, categoryDemand,
    addHandset, removeHandset, setHandsetHours, ugPhone, similarProducts,
