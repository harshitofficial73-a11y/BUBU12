...
create policy finance_enquiries_own on finance_enquiries
  for select using (account_id = current_account_id() or is_admin());
  ...
  select jsonb_build_object(
    ...
    'products', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'videos', coalesce((select jsonb_agg(jsonb_build_object('id', m.id, 'url', m.storage_path,
            'platform', m.caption) order by m.created_at)
          from media m where m.product_id = p.id and m.kind = 'video'), '[]'::jsonb),
        'brochure', (select m2.storage_path from media m2
          where m2.product_id = p.id and m2.kind = 'product_pdf'
...
returns table(
  ...
  select p.id, p.name, p.brand, p.price, p.unit, p.moq,
    ...
    coalesce((select jsonb_agg(m.storage_path order by m.created_at)
      from media m where m.product_id = p.id and m.kind = 'product' and m.approved), '[]'::jsonb),
    coalesce((select jsonb_agg(jsonb_build_object('id', m.id, 'url', m.storage_path,
        'platform', m.caption) order by m.created_at)
      from media m where m.product_id = p.id and m.kind = 'video'), '[]'::jsonb),
    (select m2.storage_path from media m2
          where m2.product_id = p.id and m2.kind = 'product_pdf'
          order by m2.created_at desc limit 1)
  from products p
