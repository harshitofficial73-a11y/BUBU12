...
(function () {
  ...
  async function loadAccount() {
    ...
    const documentFiles = await Promise.all(documents.map(async d => {
      if (!d.storage_path) return { ...d, downloadUrl: '' };
  ...
  async function searchProducts({ query = '', category = null, mine = false, limit = 100 } = {}) {
    ...
    return Promise.all(rows.map(async p => {
      const sign = async item => {
        ...
        try { return dataOf(await sb.storage.from('media').createSignedUrl(path, 3600))?.signedUrl || ''; }
        catch (_) { return ''; }
      };
      const photoItem = (p.media || []).find(m => m.kind === 'product') || null;
      const videoItem = (p.media || []).find(m => m.kind === 'product_video') || null;
      const pdfItem = (p.media || []).find(m => m.kind === 'product_pdf') || null;
      const [photo, video, pdf] = await Promise.all([sign(photoItem), sign(videoItem), sign(pdfItem)]);
      return {
      ...
      photo, img: photo, video, pdf, media: p.media || [], specs: p.product_specs || []
    }; }));
  }
  ...
  const loadCategories = async () => dataOf(await sb.from('categories').select('*').order('sort').order('name')) || [];
  ...
  const admin = {
    async applications() {
      const apps = dataOf(await sb.from('applications').select('*').eq('state', 'pending')
        .order('submitted_at')) || [];
      return Promise.all(apps.map(async app => {
        const accountResult = await sb.from('accounts').select('id,company,role,district_id,email')
          .eq('id', app.account_id).maybeSingle();
        const registrationResult = await sb.from('account_registration').select('*')
          .eq('account_id', app.account_id).maybeSingle();
        const documentsResult = await sb.from('documents').select('*').eq('account_id', app.account_id);
        return { ...app, accounts: { ...(accountResult.data || {}),
          account_registration: registrationResult.data || null,
    ...
    members: async () => dataOf(await sb.from('accounts').select('*,account_registration(overall_state),products(id,status)').order('created_at', { ascending: false })),
  ...
  async function bootstrap() {
    ...
    if (!session) return { signedIn: false, account: null, categories, products: await searchProducts({}).catch(() => []) };
    const account = await loadAccount();
    if (!account) return { signedIn: true, account: null, products: [], ownProducts: [], orders: [], conversations: [], leads: [] };
    const [products, ownProducts, conversations, leads, dashboard, leadCredit, buyerQuoteManager, adminMembers, adminAnalytics, tenders, supplierHistory, productInterests, interestNotifications] = await Promise.all([
    ...
    return { signedIn: true, account, categories, products, ownProducts, orders: [], conversations, leads, dashboard, leadCredit, buyerQuoteManager, adminMembers, adminAnalytics, tenders, supplierHistory, productInterests, interestNotifications };
  }

  window.BUBU_API = { client: sb, auth, bootstrap, loadAccount, loadCategories, searchProducts, saveProduct,
    setProductStatus, deleteProduct, offersFor, revealContact, leadBalance, purchaseLead, myBuyLeads, postRequirement,
