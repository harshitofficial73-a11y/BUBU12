...
--
-- Dropped by name first: the existing version may return a different type, and
-- "create or replace" cannot change that.
do $$
declare r record;
begin
  for r in
    select p.oid::regprocedure as sig
     ...
     where n.nspname = 'public'
       and p.proname = 'start_supplier_conversation'
  loop
